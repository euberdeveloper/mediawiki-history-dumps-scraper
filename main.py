...
def scrape(url: str, regex: str) -> list[str]:
    # do request
...
def scrape_multiple(url: str, regex: str, groups: list[str]) -> dict[str, str]:
    # do request
    response = requests.get(url)
    # get html response
    html = response.text
...
def fetch_dumps(version: str, wiki: str, /, *, start: Optional[date] = None, end: Optional[date] = None) -> list[dict[str, Any]]:
    ...
    # fetch dumps
    dumps = scrape_multiple(url, regex, groups)
    # get list of resulting objects
    result = [{
        **dump,
